...
The browser entry dynamically imports `daily-journey/index.js` only when the authorized homepage renders the game root and Play is selected. The lightweight launcher issues the protected run session before dynamically importing `game.js`; unauthorized or failed session requests therefore never request Phaser. The game imports Phaser 3.90.0's packaged `phaser-arcade-physics.min.js` distribution, which is the upstream Arcade-only production build and omits Matter Physics. Vite places it in a dedicated `daily-journey-phaser` chunk. The main app has no static import or preload dependency on either game chunk.

Production builds disable source maps and compressed-size reporting, retain esbuild minification and CSS splitting, and avoid Terser. A clean build has also been verified with a 512 MB Node heap. If Laravel Cloud still cannot build the assets, prefer increasing its build compute. The fallback is CI using pinned Node 22.18.0: run `npm ci`, `npm run test:game-ui`, and `npm run build`, then deploy the complete generated `public/build` directory and `manifest.json` from that same artifact. Never transfer `node_modules`, and never mix a manifest with assets from another build.

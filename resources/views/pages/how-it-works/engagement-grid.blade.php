<section aria-labelledby="engagement-title" class="px-4 py-20 sm:px-6 lg:px-8"><div class="mx-auto max-w-7xl"><h2 id="engagement-title" class="text-center text-3xl font-extrabold sm:text-4xl">Participation with purpose</h2>
    <div class="mt-12 grid gap-5 md:grid-cols-2">
        @foreach ([
            ['Support is free','Vote once on any Request, and support as many different Requests as you like.','votes'],
            ['People, not anonymous numbers','See who requested an idea and who helped it rise.','people'],
            ['No algorithm chooses for you','Rankings reveal demand. Creators make the final call.','decisions'],
            ['Every journey builds history','Published ideas become part of the creatorâ€™s archive and the Guideâ€™s activity.','history'],
        ] as $tile)
            <article class="rounded-3xl border border-slate-800 bg-slate-900 p-6 sm:p-8"><div class="mb-7 flex min-h-20 items-center rounded-2xl border border-slate-700/70 bg-slate-950 p-4" aria-hidden="true">
                @if($tile[2] === 'votes')<div class="flex w-full items-center justify-center"><span class="inline-flex items-center gap-2 rounded-xl bg-indigo-500 p-3 font-black">&#10003; You voted <span>&middot;</span> 43</span></div>
                ...
                @else<div class="flex w-full items-center justify-between gap-2 text-xs font-bold"><span>Requested</span><span>&rarr;</span><span>Approved</span><span>&rarr;</span><span class="text-emerald-300">Published</span></div>@endif

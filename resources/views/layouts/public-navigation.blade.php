            ...
            @auth
                ...
                <div data-header-account-control class="relative flex shrink-0 items-center" @click.outside="accountOpen = false">
                    ...
                    >
                        ...
                        @if ($creatorMenu)
                            <a href="{{ $creatorMenu['manage_requests_url'] }}" @click="accountOpen = false" role="menuitem" @class([
                                'flex min-h-11 items-center gap-3 whitespace-nowrap px-4 py-2 hover:bg-slate-100 focus:bg-slate-100 focus:outline-none dark:hover:bg-slate-800 dark:focus:bg-slate-800',
                                'bg-indigo-50 text-indigo-700 dark:bg-indigo-950/50 dark:text-indigo-300' => request()->routeIs('creators.recommendations.*'),
                            ])>
                                <svg class="size-5 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M8 6h11M8 12h11M8 18h7" />
                                    <path stroke-linecap="round" stroke-linejoin="round" d="m3.5 6 1 1 2-2M3.5 12l1 1 2-2M3.5 18l1 1 2-2" />
                                </svg>
                                Manage Requests
                            </a>

                            <a href="{{ $creatorMenu['settings_url'] }}" @click="accountOpen = false" role="menuitem" @class([
                                'flex min-h-11 items-center gap-3 whitespace-nowrap px-4 py-2 hover:bg-slate-100 focus:bg-slate-100 focus:outline-none dark:hover:bg-slate-800 dark:focus:bg-slate-800',
                                'bg-indigo-50 text-indigo-700 dark:bg-indigo-950/50 dark:text-indigo-300' => request()->routeIs('creators.settings.*'),
                            ])>
                                <svg class="size-5 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true">
                                    <path stroke-linecap="round" d="M4 7h10M18 7h2M4 17h2M10 17h10M8 14v6M16 4v6" />
                        ...
                        @endif
                        @can('creator-intelligence.manage')
                            <a href="{{ route('superadmin.creator-intelligence.overview') }}" @click="accountOpen = false" role="menuitem" class="flex min-h-11 items-center gap-3 px-4 py-2 font-bold text-indigo-700 hover:bg-indigo-50 dark:text-indigo-300 dark:hover:bg-indigo-950/40">Creator Intelligence</a>
                        @endcan
    ...
    <div x-show="open" x-cloak class="border-t border-slate-200 bg-white px-4 py-4 dark:border-slate-800 dark:bg-slate-950 md:hidden">
        <div class="grid gap-2 text-sm font-bold text-slate-700 dark:text-slate-200">
            ...
            @auth
                ...
                @if ($creatorMenu)
                    <a href="{{ $creatorMenu['manage_requests_url'] }}" @click="open = false" class="flex min-h-11 items-center gap-3 whitespace-nowrap rounded-xl px-3 py-2 hover:bg-slate-100 focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 dark:hover:bg-slate-900">
                        <svg class="size-5 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M8 6h11M8 12h11M8 18h7" />
                            <path stroke-linecap="round" stroke-linejoin="round" d="m3.5 6 1 1 2-2M3.5 12l1 1 2-2M3.5 18l1 1 2-2" />
                        </svg>
                    ...
                    <a href="{{ $creatorMenu['settings_url'] }}" @click="open = false" class="flex min-h-11 items-center gap-3 whitespace-nowrap rounded-xl px-3 py-2 hover:bg-slate-100 focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 dark:hover:bg-slate-900">
                        <svg class="size-5 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true">
                            <path stroke-linecap="round" d="M4 7h10M18 7h2M4 17h2M10 17h10M8 14v6M16 4v6" />
                            <circle cx="8" cy="17" r="2" />
                            <circle cx="16" cy="7" r="2" />
                ...
                @can('creator-intelligence.manage')<a href="{{ route('superadmin.creator-intelligence.overview') }}" class="flex min-h-11 items-center gap-3 rounded-xl px-3 py-2 font-bold text-indigo-600">Creator Intelligence</a>@endcan

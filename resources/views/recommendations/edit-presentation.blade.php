        ...
        <div class="mx-auto max-w-3xl space-y-6">
            ...
            <section class="rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900 sm:p-6">
                <form method="POST" action="{{ route('requests.presentation.update', $recommendation) }}" class="space-y-5">
                    ...
                    <div><x-input-label for="reason" value="Why this request matters"/><textarea id="reason" name="reason" rows="5" maxlength="2000" class="mt-2 block w-full rounded-xl border-slate-300 bg-white text-slate-950 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:border-slate-700 dark:bg-slate-950 dark:text-white">{{ old('reason', $recommendation->reason) }}</textarea><p class="mt-1 text-xs text-slate-500">Edit the context you want the Creator to see. 2,000 characters maximum.</p></div>

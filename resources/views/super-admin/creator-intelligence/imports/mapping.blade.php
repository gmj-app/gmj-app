<x-creator-intelligence-layout title="Column Mapping">
    ...
    @if (! in_array('platform_video_id', $batch->column_mapping ?? [], true))
        <div class="mt-4 rounded-xl bg-amber-100 p-4 text-amber-900">No YouTube Video ID (Content or Video) is mapped. Matching will rely on title and published date.</div>
    @endif
    <form method="POST" action="{{ route('superadmin.creator-intelligence.imports.mapping.update', $batch) }}" class="mt-6 overflow-hidden rounded-2xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900">
        @csrf
        @method('PUT')
        <table class="min-w-full text-sm">
            <thead><tr class="text-left text-xs uppercase text-slate-500"><th class="p-4">Source column</th><th class="p-4">Sample values</th><th class="p-4">Canonical field</th></tr></thead>

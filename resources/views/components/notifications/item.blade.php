@props(['item', 'compact' => false, 'live' => false])
<article @class(['relative flex gap-3', 'p-3' => $compact, 'rounded-2xl border p-4 sm:p-5' => !$compact, 'border-indigo-200 bg-indigo-50/70 dark:border-indigo-800 dark:bg-indigo-950/30' => !$item->isRead() && !$compact, 'border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900' => $item->isRead() && !$compact])>
    ...
    <div class="min-w-0 flex-1">
        <div class="flex items-start justify-between gap-2"><h3 class="font-extrabold text-slate-950 dark:text-white">{{ $item->title() }}</h3>@if($live)<span x-show="isHighlighted(@js($item->id()), @js(!$item->isRead()))" @if($item->isRead()) style="display: none" @endif class="mt-1 size-2 shrink-0 rounded-full bg-indigo-600" aria-label="New notification"></span>@else @unless($item->isRead())<span class="mt-1 size-2 shrink-0 rounded-full bg-indigo-600" aria-label="Unread"></span>@endunless @endif</div>

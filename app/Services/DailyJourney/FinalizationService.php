...
{
    public function __construct(private AccoladeEvaluationService $accolades, private NotificationDispatchService $notifications, private GameDayService $days) {}
    ...
    public function finalize(GameDay $day): ?GameDailyChampion
    {
        return DB::transaction(function () use ($day) {
            ...
            $best = $day->dailyBests()->orderByDesc('score')->orderByDesc('distance')->orderBy('accepted_at')->orderBy('id')->first();
            $champion = null;
            if ($best) {

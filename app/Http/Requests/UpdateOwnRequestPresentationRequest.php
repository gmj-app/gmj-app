...
class UpdateOwnRequestPresentationRequest extends FormRequest
{
    ...
    protected function prepareForValidation(): void
    {
        ...
        if ($this->exists('reason')) {
            $this->merge(['reason' => $this->clean($this->input('reason'), false)]);
        }
    }
    ...
    public function rules(): array
    {
        return [
            ...
            'reason' => ['nullable', 'string', 'max:2000', 'not_regex:/[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]/u'],
            'request_context' => ['prohibited'],
        ];
    }

    public function withValidator($validator): void
    {
        if (app(RequestIdentityComparator::class)->containsIdentityInput($this->except(['_token', '_method', 'display_title_override', 'reason']))) {
